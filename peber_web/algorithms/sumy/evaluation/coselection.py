...
def f_score(evaluated_sentences, reference_sentences, weight=1.0):
    ...
    if denominator == 0.0:
        ...
        print("F-Sscore: ( (w=%d+1) * p=%f * r=%f) / de=%f = %f" % (weight, p, r, denominator, ( ((weight + 1) * p * r) / denominator )))
...
def precision(evaluated_sentences, reference_sentences):
    ...
    :returns float:
        Returns 0.0 <= P(E) <= 1.0
    """
    # print ("Tipe: %s - %s" % (type(evaluated_sentences), type(reference_sentences)) )
...
def _divide_evaluation(numerator_sentences, denominator_sentences):
    ...
    # print ("Divide evaluation: common_count=%d/choosen_count=%d = %f" % (common_count, choosen_count, (common_count/choosen_count)))
